  ...
  <span class="addr-container">
    <span @click.stop="copyAddr" v-if="showCopy">
      <span v-if="complete" class="address-text">
        {{address}}&nbsp;
        <i class="ultfont ult-block" style="font-size: 1.1em;"></i>
      </span>
      <span v-else class="address-text">
        {{address | longStrAbbr(length)}}&nbsp;
        <i
          class="ultfont ult-block"
    ...
    <span v-else>
      <span v-if="complete" class="address-text">{{address}}&nbsp;</span>
      <span v-else class="address-text">{{address | longStrAbbr(length)}}&nbsp;</span>
    </span>
  </span>
</template>
<script>
...
export default {
  name: config.prefix + "WalletAddrSimple",
  props: {
    showCopy: {
      // 是否显示复制按钮
      type: Boolean,
