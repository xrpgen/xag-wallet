...
class Asset {
   ...
   */
  constructor(type, address, code, name, issuer, profile) {
    this.type = type;
  ...
  static insertAsset({
    ...
    let asset = new Asset(type, address, code, name, issuer, profile);
   ...
   * @returns {Array|*} 返回
   */
  static findByAddressAndType(address, type) {
    let result = instance.find({ address: address, type: type });
    return result;
  }
  /**
   * 根据地址和code删除
