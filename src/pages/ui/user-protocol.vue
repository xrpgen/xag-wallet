<template>
  <van-popup v-model="show" style="width: 100%;height: 100%;" position="bottom">
    <van-nav-bar :title="$t('setting.userProtocol')" @click-left="closePopup">
      <span slot="left">
        <i>
...
export default {
  data() {
    return {
      show: false,
      height: 0,
      protocolUrl: "static/protocol/xag-protocol-zh-CN.htm"
    };
  },
  methods: {
    closePopup() {
    ...
    showPopup() {
      this.protocolUrl = `static/protocol/xag-protocol-${this.$store.state.setting.language}.htm`;
      this.setHeight();
      this.show = true;
    },
    setHeight() {
