      ...
      <pl-content-block :offsetTop="46" :offsetBottom="45">
        ...
        >
          ...
          <small
            ...
            v-text="$t('transaction.stellarUnActivationAddressMsg')"
          ></small>
          <small
            class="text-danger"
            v-show="!trustAsset"
        ...
        <div class="item-block">
          <pl-block>
            <div style="padding: 0 0 10px;">
              ...
              <div class="pull-right">
                <span
                  ...
                  :class="{'active': form.memoType === memo.value, 'disabled' : !canSelectMemoType}"
                  :key="index"
                  v-for="(memo, index) in memoTypes"
                >
                  <span class="icon text-success" v-if="form.memoType === memo.value">
            ...
            <small
              class="text-danger"
              v-show="isNeedMemo && !form.memo"
              v-text="$t('transaction.stellarNeedMemo')"
            ></small>
...
export default {
  ...
  watch: {
    "form.receiveAddress"() {
      if (this.form.receiveAddress) {
        ...
        let config = this.$wallet.isTradingPlatformAddress(
          this.form.receiveAddress
        );
        if (config) {
          if (config["memo_type"]) {
  ...
  computed: {
    balance() {
      ...
      let frozenNative = this.getBalance(this.asset.code, this.asset.issuer)
        .frozenNative;
