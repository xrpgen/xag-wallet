<template>
  <div>
    <div style="width: 100%;" class="transfer-container send-eth">
            ...
            <div style="padding: 0 0 10px;overflow: hidden;">
              ...
              <div class="pull-right">
                <span class="small-font">
                  {{fee}}&nbsp;ether&nbsp;≈&nbsp;{{fee | market(ETH, '') | currency('', '7') | cutTail}}&nbsp;{{$store.state.setting.currencyUnit}}
                  <!--&nbsp;<van-icon name="arrow" style="vertical-align: middle;" />-->
                </span>
              </div>
            </div>
            <div class="slider">
              <van-slider bar-height="4px" @change="calcGas()" v-model="rate"></van-slider>
            </div>
      ...
      <pl-stick :offset-bottom="0">
        <van-button
          size="large"
          :loading="loading"
          :disabled="!form.amt || !form.receiveAddress"
          class="plat-btn"
          @click="firstStep"
          type="primary"
          :text="$t('common.next')"
            ...
            <van-cell-group>
              ...
              <van-cell>
                ...
                <div>
                  ...
                  >= Gas({{form.gasLimit}}) * Gas Price({{form.gasPrice}}&nbsp;gwei)</span>
                </div>
              </van-cell>
            </van-cell-group>
          </div>
...
export default {
  ...
  computed: {
    fee() {
      if (this.$wallet && this.$wallet.getInstance()) {
        let wei = new Big(this.form.gasPrice)
          .times("1000000000")
          .times(this.form.gasLimit)
          .toFixed(0);
    ...
    errMsg() {
      return {
        ...
        "insufficient funds for gas * price + value": this.$t(
          "transaction.insufficientFundsGasAndValue"
        )
      };
    }
  },
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.$store.state.account.type === this.ethereum) {
        options = {
          ...
          gasPrice: this.$wallet
            .getInstance()
            .utils.toWei(this.form.gasPrice, "gwei")
        };
      }
      ...
      this.$wallet
        ...
        .then(ret => {
          ...
          this.setTempHistory(
            ...
            options,
            () => {
              toast.message = this.$t(
                "transaction.transactionBroadcastSuccess"
              );
              setTimeout(() => {
                toast.clear();
                this.$emit("done");
    ...
    calcGas() {
      ...
      this.form.gasPrice = maxGasPrice
        .minus(minGasPrice)
        .div(100)
        .times(this.rate)
        .plus(minGasPrice)
