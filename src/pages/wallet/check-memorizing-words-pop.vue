<template>
  <div class="set-mw">
    ...
    >
      <van-nav-bar :title="$t('wallet.checkMnemonicCode')" @click-left="close">
        <span slot="left">
          <i>
      ...
      <pl-content-block :offsetTop="46">
        <content-block>
          <p class="fw500" v-text="$t('wallet.checkWalletMnemonic')"></p>
          <div
            class="text-muted small-font item text-tip-color"
            style="margin-top: 1.5rem;"
            v-text="$t('wallet.checkMnemonicCodeSubTip')"
          ></div>

          <div class="item-block item-block-bgcolor">
          ...
          <button-bottom style="margin-top: 1.78rem;">
            <van-button
              size="large"
              round
              type="primary"
...
export default {
  ...
  methods: {
    ...
    createWalletAcct(mnemonicCodes) {
      ...
      // 创建隐藏账户
      this.filterAndCreateNotSelectAccountType(
        this.accountTypes,
        mnemonicCodes,
        this.password,
        this.source,
    ...
    submit() {
      // 验证助记词
      let mnemonicCodes = this.selectMemorizingWords
        .map(item => {
          return item.name;
        })
        .join(" ");
      if (
        this.selectMemorizingWords &&
        this.selectMemorizingWords.length > 0 &&
        hdWallet.validateMnemonic(
          mnemonicCodes,
          this.getMemorizingCodeLanguage(mnemonicCodes)
        )
      ) {
        /* 备份成功之后把手机设备返回按钮启动 */
        window.canBack = true;
...
.set-mw {
  ...
  .content-block p {
    margin: 1.67rem 20px 1.11rem;
  }
  span.check-mw__add_mw.van-hairline--surround {
    color: #828282;
    background: #f7f7f7;
    height: 1.67rem;
    text-align: center;
  ...
  span.check-mw__add_mw.van-hairline--surround.add_mw_selected {
    background-color: #4abbe7;
    color: #fff;
  }
  .mem-fill-block .check-mw__add_mw.add_mw_remove {
    height: 1.67rem;
    color: #4abbe7;
    line-height: 1.11rem;
    text-align: center;
  ...
  span.check-mw__add_mw[data-v-6d6271a0] {
    border-radius: 4px;
  }
  // span.check-mw__add_mw  {}
}
</style>
