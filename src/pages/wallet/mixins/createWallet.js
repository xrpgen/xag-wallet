  ...
  methods: {
    ...
    saveDefaultAssets(accountType, account) {
      if (accountType === AccountType.ethereum) {
        ...
        console.log(tokens, account.type, accountType, account.address);
        if (tokens) {
          this.saveDefaultToken(tokens['USDT'], accountType, account.address);
          // this.saveDefaultToken(tokens['ULT'],  accountType, account.address);
          // this.saveDefaultToken(tokens['DBT'],  accountType, account.address);
        }
      }
    },
    saveDefaultToken(token, type, address) {
      if (!token) {
        return;
      }
