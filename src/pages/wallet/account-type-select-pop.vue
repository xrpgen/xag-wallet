<template>
  <div class="account-type-select">
    ...
    >
      <van-nav-bar :title="barTitle" @click-left="close" @click-right="sure">
        <span slot="left">
          <i>
      ...
      <pl-content-block :offsetTop="46">
        <content-block>
          <top-block :title="$t('wallet.selectAccountType')">
            <img src="static/img/wallet@3x.png" width="64" height="64" />
          </top-block>
          <type-select v-model="accountTypes" :clickable="false"></type-select>
        </content-block>
      </pl-content-block>
    </van-popup>
...
import { AccountTypeForSelect } from "src/wallet/constants"; //zzw
...
export default {
  ...
  methods: {
    show(source) {
      // source=1表示创建钱包，source=2表示恢复钱包, 3-表示创建其他账户钱包
      this.accountTypes = [];
      if (source === "1") {
        this.accountTypes = Object.values(AccountTypeForSelect); //zzw
      }
      this.source = source;
      ...
      try {
        ...
        if (identity) {
          ...
          this.createWalletAcctByMnemonicCode(
            this.accountTypes,
            cryptor.decryptAES(identity.value, password),
            password,
            SourceType.CREATED,
            true
          );
    ...
    deleteAccounts(identityId) {
      ...
      this.accountTypes.forEach(item => {
        this.$collecitons.account.findAndRemoveAcct({
          type: item,
          state: "D",
