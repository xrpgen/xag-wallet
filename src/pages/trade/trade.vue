<template>
  <div id="trade_page" class="trade" @click.stop="bindClick">
    <van-popup
      v-model="showPop"
      position="bottom"
      class="popup-wapper"
    ...
    >
      <van-nav-bar @click-left="close()">
        ...
        <i
          slot="right"
          @click.stop="toAddTradepair"
          class="ultfont ult-plus"
        ></i>
      </van-nav-bar>
      <pl-content-block :offsetTop="46" class="b-white">
        ...
        <div style="padding:20px 15px">
          <div class="text-muted b-white item x-small-font mb10">
            <span
              >{{ $t("trade.youAsset") }}&nbsp;&nbsp;&nbsp;{{
                balances[bsCode + bsIssuer] | cutTail
              }}&nbsp;{{ bsCode }}</span
            >
          </div>
          <div class="item">
            <van-row style="margin-bottom:10px">
              <van-col span="11" class="x-small-font">
                <span
                  v-if="bsFlag === 'buy'"
                  v-text="$t('trade.buyPrice')"
                ></span>
                <span
                  v-else-if="bsFlag === 'sell'"
                  v-text="$t('trade.sellPrice')"
                ></span>
            ...
            <van-row>
              <van-col span="11">
                <van-field
                  ...
                  style="background: #f1f8fb;padding:5px 10px;border-radius:4px"
                  :placeholder="
                    `${$t('trade.buyPrice')}(${tradepair.counterCode})`
                  "
                ></van-field>
                <van-field
                  v-if="bsFlag === 'sell'"
                  ...
                  style="background: #f1f8fb;padding:5px 10px;border-radius:4px"
                  :placeholder="
                    `${$t('trade.sellPrice')}(${tradepair.counterCode})`
                  "
                ></van-field>
              </van-col>
          ...
          <div class="item">
            <van-row>
              <van-col
                span="11"
                class="x-small-font mt10"
                v-text="$t('trade.estVal')"
              ></van-col>
            </van-row>
            <van-row>
              <van-col span="11" class="text-muted x-small-font mt5"
                >{{ expectedTradePrice | cutTail }}&nbsp;{{
                  tradepair.counterCode
                }}</van-col
              >
              <van-col span="11" offset="2">
        ...
        >
          ...
          >
            <van-button
              ...
              v-if="bsFlag === 'buy'"
              >{{
                $t("trade.buyingCode", { code: tradepair.baseCode })
              }}</van-button
            >
            <van-button
              size="large"
          ...
          <van-tabs class="tabs" v-model="tabActive" sticky @change="tabChange">
...
export default {
  ...
  computed: {
    ...
    balances() {
      ...
      if (account) {
        let bs = this.$store.state.balances[account.type + account.address];
        if (bs) {
          let result = {};
          bs.forEach(item => {
    ...
    errMsg() {
      return {
        ...
        "Insufficient reserve to create offer.": this.$t("trade.Insufficient"),
        "Ledger sequence too high.": this.$t("trade.Ledger")
      };
    },
    maxVal() {
  ...
  methods: {
    ...
    saveDefaultTradepair(account) {
      ...
      if (bs && bs.length > 1) {
        ...
        this.tradepair = {
          ...
          baseIssuer: "",
          counterCode: "XCN",
          counterIssuer: "rU343tBz1UYWMiReBPFbqACmGKAxfYBGCj"
        };
      }
    },
    toAddTradepair() {
    ...
    tradeTx(password) {
      ...
      if (this.currentAccount.type === AccountType.ripplexag) {
        selling.code = this.tradepair.counterCode;
        selling.issuer = this.tradepair.counterIssuer;
        buying.code = this.tradepair.baseCode;
        buying.issuer = this.tradepair.baseIssuer;
      ...
      this.$wallet
        ...
        .catch(err => {
          ...
          if (errMsg) {
            ...
            this.$toast(
              this.$t("trade.offerFail") + ":" + err.data.resultMessage
            );
          } else {
