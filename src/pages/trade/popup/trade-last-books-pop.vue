<template>
  <div>
    ...
    >
      <van-nav-bar :title="title" @click-left="close()">
        <span slot="left">
          <i>
          ...
          <table style="height: 100%;width: 100%;border-collapse:collapse;">
            <tr>
              <th
                colspan="1"
                rowspan="1"
                class="small-font text-muted text-left"
                v-text="$t('trade.time')"
              ></th>
              <th
                colspan="1"
                rowspan="1"
                class="small-font text-muted text-left"
                v-text="$t('trade.direction')"
              ></th>
              <th
                colspan="1"
                rowspan="1"
                class="small-font text-muted text-right"
              >{{$t('trade.amount')}}({{tradepair.baseCode}})</th>
              <th
                colspan="1"
                rowspan="1"
                class="small-font text-muted text-right"
              >{{$t('trade.price')}}({{tradepair.counterCode}})</th>
            </tr>
            <tr v-for="(item,index) in lastBooks" :key="index">
              <td
                colspan="1"
                rowspan="1"
                class="text-left small-font text-muted"
              >{{item.ledgerCloseTime}}</td>
              <td
                colspan="1"
                rowspan="1"
                class="text-left small-font"
...
export default {
  ...
  methods: {
    ...
    getLastBooks(addr) {
      this.lastBooks = [];
      let base = {
        code: this.tradepair.baseCode,
        issuer: this.tradepair.baseIssuer
      };
      let counter = {
        code: this.tradepair.counterCode,
        issuer: this.tradepair.counterIssuer
      };
      let option = { limit: 50 };
      if (addr) {
      ...
      this.$wallet
        .queryLastBook(base, counter, option)
        .then(data => {
          if (this.accountType === AccountType.stellar) {
            ...
            this.accountType === AccountType.ripple ||
            this.accountType === AccountType.ripplexag
          ) {
            if (data.count > 0) {
              let exs = [];
        ...
        .catch(err => {
          console.info(err);
          if (!err.result && err.result != 'failed') {
    ...
    processStellarLastBooks(item) {
      ...
      if (!moment(ledgerCloseDate).isSame(now)) {
        ledgerCloseTime = moment(item.ledger_close_time).format(
          "MM-DD HH:mm:SS"
        );
      }
      let result = {
        ...
        baseIsSeller:
          item.base_is_seller /*买入还是卖出，true-买入，false-卖出*/,
        counterAmount: item.counter_amount,
        counterCode:
          item.counter_asset_type === "native"
            ? "XLM"
    ...
    processRippleLastBooks(item) {
      ...
      let result = {
        ...
        counterAmount: Number(item.counter_amount)
          .toFixed(6)
          .toString(),
      ...
      if (
        this.isSameAsset(
          this.tradepair.baseCode,
          this.tradepair.baseIssuer,
          result.baseCode,
          result.baseIssuer
        ) &&
        this.isSameAsset(
          this.tradepair.counterCode,
          this.tradepair.counterIssuer,
          result.counterCode,
          result.counterIssuer
        )
      ) {
        return result;
        //交易对相反
    ...
    isSameAsset(code, issuer, code2, issuer2) {
      /*是否是当前交易对*/
      if (!issuer) issuer = "";
      if (!issuer2) issuer2 = "";
