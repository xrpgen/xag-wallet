<template>
  <div>
    ...
    >
      <van-nav-bar :title="$t('trade.addTradePair')" @click-left="close()">
        <span slot="left">
          <i>
          ...
          <van-col span="10">
            ...
            <van-row class="select-van-row" @click.native="selectTradepair('1')">
              <van-col span="5">
                ...
                <img
                  v-else
                  :src="`static/img/${pairs.baseCode}@3x.png`"
          ...
          <van-col span="10">
            ...
            <van-row class="select-van-row" @click.native="selectTradepair('2')">
              <van-col span="5">
                ...
                <img
                  v-else
                  :src="`static/img/${pairs.counterCode}@3x.png`"
                  onerror="this.src='static/img/unknown.png'"
                />
              </van-col>
              <van-col span="16">
  ...
  methods: {
    ...
    resetData() {
      ...
      if (account) {
        let assetsMap = this.getConfigAssetsMap(account.type);
        let baseAssetMap =
          assetsMap[this.pairs.baseCode + (this.pairs.baseIssuer || "")];
        if (baseAssetMap) {
          this.pairs.baseLogo = baseAssetMap.logo || "";
          this.pairs.baseName = baseAssetMap.name;
        }
        let counterAssetMap =
          assetsMap[this.pairs.counterCode + (this.pairs.counterIssuer || "")];
        if (counterAssetMap) {
    ...
    sure() {
      this.$collecitons.tradepair.findAndUpdateTradepair(
        { acctType: this.pairs.acctType, acctAddress: this.pairs.acctAddress },
        tradepair => {
    ...
    selectTradepair(type) {
      ...
      if (this.pairType === "1") {
        this.pickerBalances = this.balances.filter(item => {
          ...
          return (
            codeIssuer != this.pairs.counterCode + this.pairs.counterIssuer
          );
        });
