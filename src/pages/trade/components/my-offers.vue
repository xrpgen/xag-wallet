<template>
  <div>
    ...
    <offer-history ref="offerHistory" :type="accountType"></offer-history>
  </div>
</template>
<script>
  ...
  import {AccountType, CoinType} from "src/wallet/constants";
    ...
    methods: {
      ...
      processRippleOffers (datas) {
       ...
       processRipplexagOffers (datas) {
        ...
        datas.forEach((item) => {
          ...
          if(!buyIssuer&&buyCode===CoinType.XRP) {
            buyCode = CoinType.XAG;
          }
           if(!sellIssuer&&sellCode===CoinType.XRP) {
            sellCode = CoinType.XAG;
          }
          if (this.isSameAsset(sellCode, sellIssuer, this.tradePair.baseCode, this.tradePair.baseIssuer) && this.isSameAsset(buyCode, buyIssuer, this.tradePair.counterCode, this.tradePair.counterIssuer)) {
            isSelling = true;
            if (item.specification.direction === 'sell'){
      ...
      getOffers () {
        this.$wallet.queryOffers(this.address).then((datas) => {
          ...
          }else if (this.accountType === AccountType.ripplexag) {
            this.offers = this.processRipplexagOffers(datas);
          }
          this.startTimer();
        }).catch(() => {
          this.offers = [];
