  ...
  <div>
    <div class="b-white padding trade-offers">
      <van-row>
        ...
        >
          <div class="offer-table">
            ...
            <div
              class="small-font row"
              v-for="(item, index) in bids"
              :style="
                `background: linear-gradient(to left, #dcf6de ${item.pct}%, #fff ${item.pct}%)`
              "
              :key="index"
        ...
        <van-col span="12" style="padding-left: 3px;">
          <div class="offer-table">
            ...
            <div
              ...
              :style="
                `background: linear-gradient(to right, #fed6d8 ${item.pct}%, #fff ${item.pct}%)`
              "
              :key="index"
              @click.stop="selectOnePrice(item)"
            >
      ...
      <van-row>
        ...
        <van-col
          span="12"
          :class="accountType === 'ripplexag' ? 'text-left' : 'text-right'"
        >
          <div v-if="accountType !== 'ripplexag'" class="x-small-font dept-dw">
            ...
            <div class="selectUl x-small-font" v-if="selectPage">
              <div
                class="selectLi"
                v-for="item in pageNos"
                :key="item"
                @click.stop="toSelectPageNo(item)"
                :class="{ select: pageNo == item }"
              >
                {{ item }}
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    processBooks() {
      ...
      if (
        this.bids.length > 0 &&
        this.bids[this.bids.length - 1].depth > max_depth
      ) {
        max_depth = this.bids[this.bids.length - 1].depth;
      }
      for (let i = 0; i < this.asks.length; i++) {
        this.asks[i].pct = mathUtils.round(
          (this.asks[i].depth / max_depth) * 100,
          2
        );
      }
      for (let i = 0; i < this.bids.length; i++) {
        this.bids[i].pct = mathUtils.round(
          (this.bids[i].depth / max_depth) * 100,
          2
        );
      }
    },
