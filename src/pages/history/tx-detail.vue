<template>
  ...
  >
    <van-nav-bar style="background: transparent;" class="tx-nav-bar" @click-left="goBack">
      <span slot="title" class="text-black fw400" v-text="$t('common.detail')"></span>
      <span slot="left" class="text-black">
        <i>
    ...
    <div class="normal-block" style="overflow: hidden; margin-top:15px">
...
import txDetailRipplexag from "./tx-detail-ripplexag";
...
export default {
  components: {
    ...
    [AccountType.ripplexag]: txDetailRipplexag,
  ...
  methods: {
    ...
    to() {
      ...
      if (this.type === AccountType.ethereum) {
        ...
        title = "Wipple";
      } else if (this.type === AccountType.ripplexag) {
        link = `https://scan.xrpgen.com/#!/tx?data=${this.item.txHash}`;
        title = "Wipple";
