...
export default {
  ...
  methods: {
    setRegion() {
      let tempAllHistorys = this.$collecitons.history.findHistory(
    ...
    getOption(param, direction) {
      ...
      if (direction === "up") {
        if (
          (!param || param.minLedgerVersion === undefined) &&
          this.lastHistory
        ) {
        ...
        if (
          (!param || param.maxLedgerVersion === undefined) &&
          this.firstHistory
        ) {
