<template>
  <div>
    <van-popup
      v-model="showPop"
      position="right"
      class="popup-wapper modify-password"
      style="height: 100%;width: 100%;"
    >
      <van-nav-bar class="fw400" :title="$t('acct.modifyPwd')" @click-left="close">
        <span slot="left">
          <i>
      ...
      <div class="item-block">
        <van-cell-group :border="false">
          ...
          <van-field
            ...
            :label="$t('acct.confirmPwd')"
          />
        </van-cell-group>
      </div>
      <div class="single-btn">
        <van-button size="large" round @click="save" type="primary" :text="$t('common.save')"></van-button>
      </div>
    </van-popup>
  </div>
</template>
        ...
        if (result) {
          ...
          if (currentAccount) {
            if (
              ...
              cryptor.encryptMD5(this.form.oldWalletPwd)
            ) {
            ...
            if (this.form.oldWalletPwd !== this.form.newWalletPwd) {
              ...
              this.$collecitons.account.findAndUpdateAcct(
                { identityId: this.$store.state.account.identityId },
                account => {
                  let oldSecret = cryptor.decryptAES(
                    account.secret,
                    this.form.oldWalletPwd
                  );
                  let newSecret = cryptor.encryptAES(
                    oldSecret,
                    this.form.newWalletPwd
                  );
                  if (account.mnemonicCode) {
                    let oldMnemonicCode = cryptor.decryptAES(
                      account.mnemonicCode,
                      this.form.oldWalletPwd
                    );
                    let newMnemonicCode = cryptor.encryptAES(
                      oldMnemonicCode,
                      this.form.newWalletPwd
                    );
              ...
              /*更新身份的value*/
              this.$collecitons.identity.findAndUpdateIdentity(
                { id: this.$store.state.account.identityId },
                identity => {
                  let oldValue = cryptor.decryptAES(
                    identity.value,
                    this.form.oldWalletPwd
                  );
                  let newValue = cryptor.encryptAES(
                    oldValue,
                    this.form.newWalletPwd
                  );
