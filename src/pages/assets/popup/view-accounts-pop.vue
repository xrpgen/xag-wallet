...
export default {
  ...
  methods: {
    ...
    updateCurrentAcct() {
      let createIdentity = this.$collecitons.identity.findBySource(
        SourceType.CREATED
      );
      if (createIdentity && createIdentity.length > 0) {
        this.createAccounts = this.$collecitons.account.findByIdentityId(
          createIdentity[0].id
        );
      }
    ...
    updateImportAcct() {
      this.importAccounts = [];
      let importIdentity = this.$collecitons.identity.findBySource(
        SourceType.IMPORT
      );
      if (importIdentity && importIdentity.length > 0) {
        importIdentity.forEach(item => {
          this.importAccounts.push(
            ...this.$collecitons.account.findByIdentityId(item.id)
          );
        });
      }
      this.$emit("updateAcct");
