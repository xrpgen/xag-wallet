<template>
  ...
  >
    ...
    <van-cell-group class="margin-top shadow-bottom">
      <van-field
        maxlength="80"
        :placeholder="$t('assets.enterContractName')"
        clearable
        name="name"
        v-model="form.name"
        :label="$t('assets.contractName')"
      />
      <van-field
        maxlength="80"
        :placeholder="$t('assets.enterContractSymbol')"
        clearable
        name="symbol"
        v-model="form.symbol"
        :label="$t('assets.contractSymbol')"
      />
      <small
        class="text-danger padding-left"
        v-show="!assetExist"
      >{{$t('assets.contractAssetExist')}}</small>
      <van-field
        maxlength="36"
        type="number"
        :placeholder="$t('assets.enterContractDecimals')"
        clearable
        name="decimals"
        v-model="form.decimals"
        :label="$t('assets.contractDecimals')"
      />
      <van-field
        type="textarea"
        class="no-border"
        autosize
        maxlength="80"
        :placeholder="$t('assets.enterAssetAddress')"
        clearable
        name="address"
        v-model="form.address"
        :label="$t('assets.assetAddress')"
      />
      <small
        class="text-danger padding-left"
        v-show="!addressValid"
      >{{$t('assets.contractAddressValid')}}</small>
      <small
        class="text-danger padding-left"
        v-show="!addressExist"
      >{{$t('assets.contractAddressExist')}}</small>
    </van-cell-group>
...
export default {
  ...
  watch: {
    "form.address"() {
      if (this.form.address) {
        ...
        if (
          !coins[AccountType.ethereum].wallet.isValidAddress(this.form.address)
        ) {
  ...
  methods: {
    ...
    saveToken() {
      ...
      // 保存数据库操作
      this.loading = true;
      coins[AccountType.ethereum].wallet
        .getContractAbi(this.form.address)
        .then(ret => {
          if (ret && ret.length > 0) {
